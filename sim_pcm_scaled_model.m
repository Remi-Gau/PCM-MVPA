...
for iTheta = 1:size(theta,1)
    ...
    figure(1)
    scatter(...
        Y{1}(logical(condVec(:,1)),3),...
        Y{1}(logical(condVec(:,1)),6), '.b')
    hold on
    scatter(...
        Y{1}(logical(condVec(:,2)),3),...
        Y{1}(logical(condVec(:,2)),6), '.r')
    ...
    tmp = zscore(Y{1},0,2);
    scatter(...
        tmp(logical(condVec(:,1)),3),...
        tmp(logical(condVec(:,1)),6), '.b')
    hold on
    scatter(...
        tmp(logical(condVec(:,2)),3),...
        tmp(logical(condVec(:,2)),6), '.r')
